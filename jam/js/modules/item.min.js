...
get_where_list(whereDef){var field,field_name,filter_type,filter_str,result=[];for(field_name in whereDef){if(whereDef.hasOwnProperty(field_name)){let field_arg=field_name,value=whereDef[field_name],arr=field_name.split('__');if(typeof value==='object'&&value!==null){result.push(this.get_where_list(value));}
else{field_name=arr[0]
...
return options;}
_do_open(offset,async,params,open_empty,callback){var self=this,i,filters,data;params=$.extend(true,{},params);for(i=0;i<params.__filters.length;i++){let filter=params.__filters[i];if(filter[0]instanceof Array){filter.forEach(function(or_filter){or_filter.length=3;});}
...
if(this.is_modified()||this.is_new()){if(this.change_log){this.change_log.log_change();}}
...
create_detail_table(container,options){var self=this,details=this.view_options.view_details,tab_changed=function(tab_index){let index=+tab_index.replace('tab','');if(self._visible_detail){self._visible_detail.close();}
...
for(i=0;i<details.length;i++){let detail=this.find(details[i]),detail_container=container,height_delta=0;if(details.length>1){detail_container=task.add_tab(container,detail.item_caption,{tab_id:detail.item_name+'_detail_tab'+i,on_click:tab_changed});height_delta=38;}
...
for(i=0;i<details.length;i++){detail=this.find(details[i]);detail_container=container;if(details.length>1){detail_container=task.add_tab(container,detail.item_caption,{tab_id:detail.item_name+'_detail_view_tab'+i});}
