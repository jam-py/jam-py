...
for(var j=0;j<consts.field_attr.length;j++){attr=consts.field_attr[j];if(attr.charAt(0)==='_'){attr=attr.substr(1);}
if(attr==='data_type'){attr='field_type'}
val=fields[i][attr]
switch(attr){case'ID':val=i+1;break;case'field_type':field_type=fields[i]['field_type']
val=consts.field_type_names.indexOf(field_type);if(val<1){val=1;}
...
search(){var args=this._check_args(arguments),callback=args['function'],paginating=args['boolean'],field_name=arguments[0],text=arguments[1].trim(),search_text=text,filter,field,filter,filter_type,i,j,index,ids,substr,str,found,lookup_values,params={};if(arguments.length>2&&typeof arguments[2]==="string"){filter=arguments[2];filter_type=consts.filter_value.indexOf(filter)+1;}
...
else{self._do_after_delete_record(master_changing,callback);}});},function(){self.hide_message(mess);},null,{hide:false});}else{this.warning(task.language.no_record);}}}
