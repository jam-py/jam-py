...
(function($) {
    ...
    AbsrtactItem.prototype = {
        ...
        init: function(info) {
            ...
            for (; i < len; i++) {
                ...
                child = this.addChild(item_info[0], item_info[1],
                    item_info[2], item_info[3], item_info[4], item_info[5]);
                if (child.initAttr) {
        ...
        findTemplate: function(suffix, options) {
            var result,
                template,
                name,
                item = this;
            if (options.template_class) {
                template = this.task.templates.find("." + options.template_class);
            }
            else {
                while (true) {
                    name = item.item_name;
        ...
        create_form: function(suffix, options) {
            ...
            options.item_options = this[suffix + '_options'];
            this[formName] = $("<div></div>").append(this.findTemplate(suffix, options.item_options));
            if (!options.container) {
                this[formName] = this.makeFormModal(this[formName], options.item_options);
            }
            if (this[formName]) {
                ...
                if (options.container) {
                    ...
                    if (this[formName].hasClass("modal")) {
                        ...
                        this[formName].on("keyup." + keySuffix, function(e) {
                            ...
                            e.stopPropagation();
                            datepicker = self[formName].find('.datepicker')
        ...
        message: function(mess, options) {
            ...
            $element.modal({width: options.width, height: options.height, keyboard: options.close_on_escape});
            return $element;
        },
        ...
        question: function(mess, yesCallback, noCallback) {
            ...
            return this.message(mess, {buttons: buttons, margin: "20px 20px", text_center: true, center_buttons: true});
        },
        ...
        yes_no_cancel: function(mess, yesCallback, noCallback, cancelCallback) {
            ...
            return this.message(mess, {buttons: buttons, margin: "20px 20px", text_center: true, width: 500, center_buttons: true});
        },
    ...
    $.extend(Task.prototype, {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.error) {
                        ...
                        else if (self.ID > 0 && data.result.version && self.version && data.result.version !== self.version) {
                            if (!self.task._version_changed) {
                                self.task._version_changed = true;
                                self.message('<h4>' + language.apply_changes + '</h4>', {margin: '50px 0px', width: 500,  text_center: true});
                            }
                        }
                        if (callback) {
        ...
        load: function() {
            var logingInfo,
                ...
                info;
            if (!this.user_id) {
                this.user_id = this.read_cookie('user_id');
            }
            this.send_request('init_client', null, function(info) {
                ...
                if (info.task[5]) {
                    self.js_filename = 'js/' + info.task[5];
                }
                self.task = self;
        ...
        login: function() {
            ...
            $form.find("#login-btn").click(function(e) {
                ...
                self.create_cookie('user_id', self.user_id, 0.5);
                if (self.user_id) {
                    if ($form) {
                        $form.modal('hide');
                    }
            ...
            if (this.log_changes()) {
                ...
                if (this.item.item_state === consts.STATE_BROWSE) {
                    if ((this.item.get_record_status() === consts.RECORD_UNCHANGED) ||
                        (this.item.get_record_status() === consts.RECORD_DETAILS_MODIFIED && record_log['unmodified_record'] === null)) {
...
//                        'unmodified_record': record_log['unmodified_record'],
        ...
        store_record_log: function() {
            ...
            if (this.log_changes()) {
                ...
                for (var i = 0; i < this.item.details.length; i++) {
                    ...
                    if (!detail.disabled && detail._records) {
    ...
    $.extend(Item.prototype, {
        ...
        set_select_fields: function(field_list) {
            ...
            if (this.master) {
                if (!this.disabled && this.master.record_count() > 0) {
                    ...
                    if (records !== undefined) {
                        this.do_before_open(expanded, fields,
        ...
        post: function(callback) {
            ...
            else if (this.get_record_status() === consts.RECORD_UNCHANGED) {
                this.change_log.remove_record_log();
            }
            this._modified = false;
            this.set_state(consts.STATE_BROWSE);
            ...
            if (!this._valid_record()) {
                this.update_controls(consts.UPDATE_DELETE);
                this._search_record(this.get_rec_no(), 0);
            }
        },
        ...
        apply: function() {
            ...
            if (!this.master && !this.change_log.is_empty_obj(changes.data)) {
    ...
    $.extend(Item.prototype, {
        ...
        find_last: function() {
            this._search_record(this._records.length, -1);
        },

        find_next: function() {
            this._search_record(this.get_rec_no(), 1);
        },
    ...
    $.extend(Item.prototype, {
        ...
        get_status_text: function() {
            var result = '',
                status = '';
            this.each_filter(function(filter, index) {
    ...
    Field.prototype = {
        ...
        do_on_change_lookup_field: function(lookup_value, slave_field_values) {
            ...
            if (this.lookup_item) {
                if (this.master_field) {
                    ...
                    if (this.owner) {
                        this.owner.each_field(function(field, i) {
        ...
        do_before_changed: function(new_value, new_lookup_value) {
            if (this.field_kind === consts.ITEM_FIELD) {
                ...
                if (this.owner.on_before_field_changed) {
                    this.owner.on_before_field_changed.call(this.owner, this, new_value, new_lookup_value);
                }
            }
        },
        ...
        set_value: function(value, lookup_value, slave_field_values, lookup_item) {
            ...
            if (this.get_raw_value() !== this.new_value) {
                this.do_before_changed(this.new_value, lookup_value);
                try {
        ...
        get_read_only: function() {
            ...
            if (this.owner && this.owner._parent_read_only && this.owner.get_read_only()) {
    ...
    DBTable.prototype = {
        ...
        createTable: function() {
            ...
            this.$element.on('mousedown.grid-title', 'table.outer-table thead tr:first th', function(e) {
                ...
                if ($this.outerWidth() - e.offsetX < 8 && this !== lastCell) {
                    ...
                    if (self.item.master || !self.item.paginate) {
                        ...
                        self.item.open({params: self.item._open_params, offset: 0});
                    }
                }
            });
            ...
            $td = table.find('td')
            this.td_height_margin = parseInt($td.css('padding-top'), 10) + parseInt($td.css('padding-bottom'), 10) +
                parseInt($td.css('margin-top'), 10) + parseInt($td.css('margin-bottom'), 10) +
                parseInt($td.css('border-top-width'), 10);
            ...
            if (this.item.paginate) {
                if (this.options.row_line_count > 1) {
                    this.row_count = Math.floor((this.$scroll_div.height() - 1) / (this.textHeight * this.options.row_line_count + this.td_height_margin + 1));
                }
                else if (this.options.expand_selected_row && this.options.row_line_count === 1) {
...
//                    this.selected_row.find('td').addClass(selClassName);
        ...
        select_row: function($row) {
            ...
            this.hide_selection();
            if (this.selected_row && this.options.expand_selected_row && this.options.row_line_count === 1) {
                this.selected_row.find('tr, div').css('height', textHeight);
            }
            this.selected_row = $row;
            this.show_selection();
            if (this.options.expand_selected_row && this.options.row_line_count === 1) {
        ...
        newColumn: function(columnName, align, text, index, setFieldWidth) {
            ...
            if (this.textHeight && this.options.row_line_count >= 1) {
    ...
    window.task.constructors = {task: Task, group: Group, item: Item, detail: Detail};
