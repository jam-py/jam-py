...
function Events0() { // app_builder 
	...
	function on_page_loaded(task)  {
		...
		if (!task.sys_tasks.f_db_type.value) {
			fields = ['f_name', 'f_item_name', 'f_db_type', 'f_python_library', 'f_server', 
	...
	function on_view_form_shown(item) {
		...
		if (item.active && item.view_table) {
	...
	function set_project_params(task, caption) {
		...
		task.sys_params.edit_options.title = caption + task.help_badge('https://jampy-docs-v7.readthedocs.io/en/latest/admin/project/database.html');
		task.sys_params.edit_record();
	}
...
function Events2() { // sys_roles 
	...
	function on_view_form_created(item) {
		...
		item.sys_privileges.on_after_scroll = update_fields;
		
		item.sys_field_privileges.set_order_by(['field']);
		item.sys_field_privileges.create_table(item.view_form.find("#priv-panel .view-fields"),
			{
				height: 200,
				sortable: true,
				dblclick_edit: false
		...
		item.view_form.find('.dbtable.sys_field_privileges .inner-table').on('click', 'td', function() {
			let field_name = $(this).data('field_name');
			field_priv_clicked(item, field_name);
		});
	}
	
	function field_priv_clicked(item, field_name) {
		let pr = item.sys_field_privileges,
			field = pr.field_by_name(field_name);
		if (field.field_type === "boolean") {
			field.data = !field.value;
			let id = task.server('server_change_field_privilege', 
				[pr.id.value, pr.owner_rec_id.value, pr.item.value, pr.field.value, field_name, field.value]);
			pr.id.data = id;
			pr.update_controls();
			task.sys_roles.server('roles_changed');
		}
	}
	
	function select_all_clicked(item, value) {
		var detail = item.sys_privileges,
			on_after_scroll = detail.on_after_scroll,
		...
		try {
			...
			detail.on_after_scroll = undefined;
			...
			detail.on_after_scroll = on_after_scroll;
		}
		if (item.is_changing()) {
			item.post();
			item.apply();
		}
		detail.on_after_scroll(detail);
	}
	...
	function update_fields(item) {
		if (item.rec_count && item.owner.id.value) {
			item.owner.sys_field_privileges.set_order_by(['field']);		
			item.owner.sys_field_privileges.set_where({owner_rec_id: item.owner_rec_id.value, item: item.item_id.value});
			item.owner.sys_field_privileges.open();
		}
		else {
			item.owner.sys_field_privileges.open({open_empty: true});
		}
	}
	this.on_view_form_created = on_view_form_created;
	this.field_priv_clicked = field_priv_clicked;
...
function Events3() { // sys_items 
	...
	function init_fields(item) {
		...
		if (task.item_tree.type_id.value === task.item_types.TASKS_TYPE) {
			...
			item.view_options.fields = ['id', 'f_name', 'f_item_name', 'f_table_name', 'f_master_applies'];
			item.edit_options.fields = ['f_name', 'f_item_name', 'f_master_applies'];
		}
	}
	...
	function init_view_table(item, options) {
		let types = item.task.item_types;
		if ([types.ITEMS_TYPE, types.TABLES_TYPE, types.REPORTS_TYPE]
			.indexOf(item.task.item_tree.type_id.value) === -1) {
			options.multiselect = false;
			item.selections = undefined;
		}
		else {
	...
	function on_view_form_created(item) {
		...
		item.view_form.find('#new-report-group-btn').hide();
		if (parent_type_id === types.TASKS_TYPE) {
			...
			item.view_form.find('#copy-btn').hide();
			item.view_form.find('#move-btn').hide();		
			item.view_form.find('#new-report-group-btn').show();
		}
		else if (type_id === types.REPORT_TYPE) {
			item.view_form.find('#copy-btn').hide();
		}
		else if (type_id === types.DETAIL_TYPE) {
		...
		item.view_form.find("#new-btn").off('click.task').on('click', function() {
			if (parent_type_id === types.TASK_TYPE) {
				item.append();
				item.type_id.value = types.ITEMS_TYPE;
		...
		item.view_form.find("#new-report-group-btn").on('click', function() {
			item.append();
			item.type_id.value = types.REPORTS_TYPE;
			item.create_edit_form();
		});
		item.view_form.find("#copy-btn").on('click', function() {
			let item_id = item.id.value,
				fields = {},
				int_fields = ['f_primary_key', 'f_deleted_flag', 'f_master_id',  
					'f_master_rec_id'];
	
				;
			item.each_field(function(f) {
				fields[f.field_name] = [f.data, f.lookup_data]
			});
			item.append();
			item.each_field(function(f) {
				if (!f.system_field() && int_fields.indexOf(f.field_name) === -1) {
					f.data = fields[f.field_name][0];
					f.lookup_data = fields[f.field_name][1];
				}
			});
			
			item.f_copy_of.value = item_id;
			item.sys_fields.open();
			item.add_fields_to_copy(item, true);
		});
		item.view_form.find('#up-btn').click(function(e) {
			e.preventDefault();
			item.task.move_record_up(item);
			save_order(item);
		});
		item.view_form.find('#down-btn').click(function(e) {
			e.preventDefault();
			item.task.move_record_down(item);
			save_order(item);
		});
		item.view_form.find('#move-btn').click(function(e) {
			e.preventDefault();
			move_to_group(item);
		});
	}
	...
	function on_edit_form_created(item) {
		...
		item.edit_form.find("#add-fields-btn").hide();
		if (item.item_name === 'sys_items') {
			if (!item.can_modify) {
	...
	function add_fields_to_copy(item, creating) {
		...
		if (copy_source.rec_count) {
			...
			if (creating) {
				...
				fields.view_options.title = 'Select fields to add';
				while (!fields.eof()) {
					if (clone.locate('f_db_field_name', fields.f_db_field_name.value) || fields.f_calc_item.value) {
						fields.delete()
					}
					else {
						fields.next();
				...
				if (flds.selections.length) {
					flds.each(function(fld) {
						if (flds.selections.indexOf(fld.id.value) !== -1) {
							item.sys_fields.append();
							item.sys_fields.each_field(function(f) {
								if (f.field_name !== 'id') {
									f.data = fld.field_by_name(f.field_name).data;
	...
	function on_field_changed(field, lookup_item) {
		var copy,
			...
			names,
			item = field.owner
		if (!item.f_copy_of.value && item.is_new() && item.type_id.value != item.task.item_types.DETAIL_TYPE) {
	...
	function get_fields_list(task) {
		...
		if (item.table_id.value === 0) {
			fields.set_where({owner_rec_id: item.id.value});
		}
		else {
			fields.set_where({owner_rec_id: item.table_id.value});		
		}
		fields.set_order_by(['f_field_name']);
	...
	function get_detail_source_list(item) {
		...
		let owner_id = item.id.value;
		if (item.f_copy_of.value) {
			owner_id = item.f_copy_of.value;
		}
		fields.set_where({f_object: owner_id, f_master_field__isnull: true })
		fields.open()
		fields.each(function(f) {
			let item_name = item.field_by_id(f.owner_rec_id.value, 'f_item_name')
			result.push([f.owner_rec_id.value, item_name, f.f_field_name.value, f.id.value]);
		})
		return result;
	}
	...
	function get_detail_dest_list(item) {
		...
		details.each(function(d) {
			let field_id = d.f_master_field.value,
				field_name = '';
			if (field_id) {
				field_name = fields.field_by_id(field_id, 'f_field_name');
			}
			result.push([d.table_id.value, field_name, field_id]);
		});
		return result;
	}
	...
	function report_params_setup(item) {
		var fields = ['f_name', 'f_param_name','f_data_type', 'f_object', 'f_object_field', 'f_multi_select',
			'f_lookup_values', 'f_enable_typehead', 'f_required', 'f_alignment', 'f_visible'];
	...
	function privileges_setup(item) {
		...
		priv.view_options.fields = ['owner_rec_id', 'f_can_view', 'f_can_create', 'f_can_edit', 'f_can_delete']
		priv.view();
	}
	...
	function on_after_apply(item) {
		...
		if (item.rec_count && item.cur_record_count && item.cur_record_count !== item.rec_count) {
			if (item.type_id.value === item.task.item_types.ITEMS_TYPE ||
				item.type_id.value === item.task.item_types.TABLES_TYPE ||
				item.type_id.value === item.task.item_types.REPORTS_TYPE) {
				item.task.refresh_tree(item.task);
	...
	function on_before_append(item) {
		item.cur_record_count = item.rec_count;
	}
	...
	function move_to_group(item) {
		let copy = item.copy({handlers: false, details: false}),
			groups = item.copy({handlers: false, details: false}),
			selections = item.selections,
			types = task.item_types;
		if (!item.rec_count) {
			return
		}
		copy.open({fields: ['parent'], open_empty: true});
		copy.edit_options.template_class = 'catalogs-edit';
		copy.edit_options.fields = ['parent'];
		copy.edit_options.title = 'Move item to group'
		copy.edit_options.width = 450;
		copy.parent.required = true;
		copy.parent.field_caption = 'New group'
		if (item.type_id.value === types.REPORT_TYPE) {
			groups.set_where({type_id: types.REPORTS_TYPE});		
		}
		else {
			groups.set_where({type_id__in: [types.ITEMS_TYPE, types.TABLES_TYPE]});
		}
		groups.open()
		copy.parent.lookup_values = [];
		groups.each(function(g) {
			if (g.id.value !== item.parent.value) {
				copy.parent.lookup_values.push([g.id.value, g.f_name.value])
			}
		})
		...
		copy.on_edit_form_created = function(c) {
			c.create_inputs(c.edit_form.find(".edit-body"));
			c.edit_form.find("#cancel-btn")
				.text(c.task.language.cancel)
				.on('click.task', function(e) {
					c.cancel_edit(e); 
				});
			c.edit_form.find('#ok-btn').on('click', function() {
				try {
					c.post();
					task.server('server_move_to_group', [c.parent.value, selections]);
					item.selections = [];
					item.refresh_page();
					c.close_edit_form();
				}
...
function Events8() { // app_builder.catalogs.sys_params 
	...
	function on_after_apply(item) {
		if (item.task.init_project && item.f_language.value) {
			if (item.f_language.value) {
				item.task.server('server_set_project_langage', [item.f_language.value]);
			}
			location.reload();
		}
		if (item.f_production.value) {	
			item.task.server('server_set_production', function() {
				location.reload();			
			});
		}
		else {
...
function Events9() { // app_builder.catalogs.sys_langs 
	...
	function on_view_form_created(item) {
		...
		item.view_form.find('#import-btn').click(function(e) {
			e.preventDefault();
		...
		item.view_form.find('#export-btn').click(function(e) {
			e.preventDefault();
	...
	function on_edit_form_created(item) {
		...
		item.edit_form.find('.modal-footer #new-btn').click(function(e) {
		...
		item.edit_form.find('.modal-footer #delete-btn').click(function(e) {
			e.preventDefault();
...
function Events00() { // app_builder.catalogs.sys_tasks 
	...
	function update_db_type(item, db_type, lib) {
		...
		if (db_type) {
			item.f_dsn.read_only = !db_params.dsn;
	...
	function on_edit_form_shown(item) {
		item.prev_db_type = item.f_db_type.value;
		item.edit_form.find('textarea.f_dsn').height('7.25em');
...
function Events05() { // app_builder.catalogs.sys_fields_editor 
	...
	function set_cur_media(item, media) {
		...
		if (item.type === 'view') {
			...
			if (tabs.length === 1 && tabs[0][TAB_NAME] === '') {
				item.view_form.find('#field-tabs > li a').html('&nbsp');
			}
			else {
	...
	function update_devices(item) {
		for (let i = 0; i < 3; i++) {
			let a = item.view_form.find('#media-tabs #media' + i);
			if (item.dest_object[i] === undefined) {
				a.hide();
			}		
			else {
				a.show();
			}
			item.view_form.find('#media-tabs a').removeClass('active');
			item.view_form.find('#media-tabs #media0').addClass('active');
		}
	}
	
	function update_media(item, media, selected) {
		if (selected) {
			if (item.dest_object[media] === undefined) {
				let source_media = media - 1;
				if (item.dest_object[source_media] === undefined)  {
					source_media = 0;
				}
				item.dest_object[media] = $.extend(true, {}, item.dest_object[source_media]);
				// if (item.type === 'view') { 
				//	 item.dest_object[media] = ['', {}, [], {}, []];
				// } else if (item.type === 'edit') { 
				//	 item.dest_object[media] = ['', {}, [], [['', [[{}, [], '']]]]];
			...
			item.dest_object[media] = undefined;
		}
	}
	
	function select_media(item) {
		let copy = item.copy({handlers: false});
		copy.set_fields(['id', 'name']);	
		copy.name.field_caption = 'Device';
		copy.view_options.title = 'Select devices';
		copy.view_options.template_class = "journals-view";
		copy.view_options.history_button = false;
		copy.view_options.enable_search = false;
		copy.view_options.width = '20rem';
		copy.table_options.multiselect = true;	
		copy.table_options.fields= ['name'];
		copy.table_options.height = '12rem';
		copy.table_options.dblclick_edit = false;
		copy.paginate = false;
		copy.on_view_form_created = function(c) {
			c.view_table = c.create_table(c.view_form.find(".view-table"));
			c.view_form.find('.form-footer').hide();
		};
		copy.on_view_form_closed = function(c) {
			update_media(item, 1, c.selections.includes(1));
			update_media(item, 2, c.selections.includes(2));
			update_devices(item);
		};
		...
		if (item.dest_object[2] !== undefined) {
			selections.push(2); 
		}
	...
	function set_editor_type(item) {
		...
		if (item.type === 'edit' || item.type === 'view') {
			...
			item.view_form.find('#media-pills').show();
			set_cur_media(item, 0);
			item.view_form.find('#media-tabs').show();
			item.view_form.on('click', '#media-tabs > a.nav-link',
				function(e) {
					e.preventDefault();
					$('#media-tabs > a').removeClass('active');
					$(this).addClass('active');
					set_cur_media(item, $(this).data('media'));
				}
			);
			item.view_form.find('#change-devices-btn').click(function(e) {
			   e.preventDefault();
			   select_media(item);
			});
			update_devices(item);
	...
	function change_ed(item, op) {
		...
		if (op === 'del_tab') {
			if (item.cur_media[TABS].length === 1) {
				...
				item.view_form.find('#field-tabs > li a').html('&nbsp').removeClass('active');
			}
			else {
						...
						if (it.name.value) {
							if (op === 'new_tab') {
								if (item.cur_media[TABS].length === 1 && item.cur_media[TABS][0][TAB_NAME] === '') {
									...
									item.view_form.find('#field-tabs > li > a').addClass('active').show();
								}
								else {
									tab = [it.name.value, [[{}, [], '']]];
									item.view_form.find('#field-tabs > li > a').removeClass('active');
	...
	function set_layout(item) {
		...
		item.view_form.find("#up-btn").attr('tabindex', -1).click(function(e) {
			e.preventDefault();
			item.task.move_record_up(item.dest);
		});
		item.view_form.find("#down-btn").attr('tabindex', -1).click(function(e) {
			e.preventDefault();
			item.task.move_record_down(item.dest);
		});
		item.view_form.find("#left-btn").attr('tabindex', -1).click(function(e) {
			e.preventDefault();
			move_left(item);
		});
		item.view_form.find("#right-btn").attr('tabindex', -1).click(function(e) {
			e.preventDefault();
			move_right(item);
		});
	...
	function move_hor(item, source, dest) {
		if (source.record_count()) {
			dest.append();
			dest.each_field(function(f) {
				let field = source.field_by_name(f.field_name);
				if (field) {
					f.value = field.value;
				}
			})
	...
	this.update_media = update_media;
	this.select_media = select_media;
...
function Events08() { // app_builder.catalogs.sys_languages 
	...
	function on_view_form_created(item) {
		...
		item.view_form.find("#select-btn").click(function(e) {
			e.preventDefault();
...
function Events22() { // app_builder.details.sys_indices 
	...
	function edit_index(item, is_new) {
		...
		function get_fields_list(task) {
			...
			fields.set_where({owner_rec_id: item.id.value});
			fields.set_order_by(['f_field_name']);
			fields.open({fields: ['id', 'f_field_name', 'f_master_field', 'f_calc_item']});
			fields.each(function (f) {
				if (!f.f_master_field.value && !f.f_calc_item.value) {
					list.push([f.id.value, f.f_field_name.value]);
				}
			});
			return list;
	...
	function on_field_select_value(field, lookup_item) {
		...
		lookup_item.set_where({owner_rec_id: item.id.value});
...
function Events23() { // app_builder.details.sys_filters 
	...
	function on_field_select_value(field, lookup_item) {
		...
		if (field.field_name === 'f_field') {
			items = item.copy();
			lookup_item.set_where({
				owner_rec_id: item.task.sys_items.id.value,
...
function Events24() { // app_builder.details.sys_privileges 
	...
	function on_view_form_created(item) {
		...
		item.sys_field_privileges = task.sys_field_privileges.copy();
		item.sys_field_privileges.set_order_by(['field']);
		item.sys_field_privileges.create_table(item.view_form.find(".view-fields"),
			{
				height: 200,
				sortable: true,
				dblclick_edit: false
		...
		item.view_form.find('.dbtable.sys_field_privileges .inner-table').on('click', 'td', function() {
			let field_name = $(this).data('field_name');
			task.sys_roles.field_priv_clicked(item, field_name);
		});
	}
	
	function select_all_clicked(item, value) {
		var rec_no = item.rec_no,
			on_after_scroll = item.on_after_scroll;
	
		if (value === undefined) {
			value = true;
		}
		try {
			item.on_after_scroll = undefined;
			item.disable_controls();
			...
			item.rec_no = rec_no;
			item.enable_controls();
			item.on_after_scroll = on_after_scroll;
		}
	}
	...
	function update_fields(item) {
		if (item.rec_count) {
			item.sys_field_privileges.set_order_by(['field']);		
			item.sys_field_privileges.set_where({owner_rec_id: item.owner_rec_id.value, item: task.sys_items.id.value});
			item.sys_field_privileges.open();
		}
		else {
			item.sys_field_privileges.open({open_empty: true});
		}
	}
	this.init_view_table = init_view_table;
	this.on_view_form_created = on_view_form_created;
...
function Events25() { // app_builder.details.sys_field_lookups 
	...
	function on_field_validate(field) {
		...
		let item = field.owner,
			...
			unique = true;	
		clone.each(function(c) {
			if (c.f_value.value === item.f_value.value && c.rec_no !== item.rec_no) {
				unique = false;
				return false;
			}
		});
	...
	function on_after_append(item) {
		let clone = item.clone(),
			val = 0;
		clone.each(function(c) {
			if (c.f_value.value > val) {
				val = c.f_value.value;
			}
		});
		item.f_value.value = val + 1;
	}
	this.on_field_validate = on_field_validate;
	this.on_after_append = on_after_append;
}
...
function Events26() { // app_builder.catalogs.sys_items.sys_fields 
	...
	function on_field_select_value(field, lookup_item) {
		...
		if (field === item.f_object) {
			where = {};
			if (item.owner === task.sys_items) {
				if (item.owner.type_id.value === task.item_types.TASK_TYPE) {
			...
			where.type_id__in = [item.task.item_types.ITEM_TYPE, task.item_types.TABLE_TYPE];
			where.table_id = 0;
			where.f_copy_of__isnull = true;
			lookup_item.set_where(where)
			lookup_item.set_order_by(['f_item_name']);
		}
		...
		if (field === item.f_calc_item) {
			let ids = [],
				where = {id: -100},
				owner_id = item.owner.id.value;
			if (item.owner.f_copy_of.value) {
				owner_id = item.owner.f_copy_of.value;
			}
			fields = task.sys_fields.copy();
			fields.set_where({f_object: owner_id, f_master_field__isnull: true })
			fields.open()
			fields.each(function(f) {
				ids.push(f.owner_rec_id.value);
			})
			if (ids.length) {
				where = {id__in: ids}
			}
		...
		else if (field === item.f_calc_lookup_field) {
			if (item.f_calc_item.value) {
				let owner_id = item.owner.id.value;
				if (item.owner.f_copy_of.value) {
					owner_id = item.owner.f_copy_of.value;
				}
				lookup_item.set_where({
					owner_rec_id: item.f_calc_item.value,
					f_object: owner_id,
					f_master_field__isnull: true
				});
				lookup_item.set_order_by(['f_field_name']);
		...
		else if (field === item.f_calc_field) {
			if (item.f_calc_item.value) {
				let owner_id = item.owner.id.value;
				if (item.owner.f_copy_of.value) {
					owner_id = item.owner.f_copy_of.value;
	...
	function check_valid_field_name(item, field_name) {
		...
		if (!item.owner.valid_identifier(field_name)) {
			return task.language.invalid_field_name + ' - "' + field_name + '"';
		}
		...
		if (!error) {
			check_item = new task.constructors.item();
			if (check_item[field_name] !== undefined) {
				error = task.language.reserved_word  + ' - "' + field_name + '"';
			}
		}
		return error;
	}
	...
	function on_field_validate(field) {
		...
		if (field.field_name === 'f_field_name') {
			...
			if (item.f_data_type.value === task.consts.KEYS && !field.value) {
				return 'For keys field a lookup item must be set';
			}
		}
		else if (field.field_name === 'f_object_field') {
			if (item.f_object.value && !field.value) {
				return task.language.object_field_required;
			}
		}
		else if (field.field_name === 'f_data_type') {
			if (!item.f_data_type.value) {
				return task.language.type_is_required;
			}
		}
		else if (field.field_name === 'f_size' && item.f_data_type.value === task.consts.TEXT && !field.value) {
			return task.language.size_is_required;
		}
		else if (field.field_name === 'f_default_value') {
			if (field.value) {
				if (item.f_data_type.value === task.consts.INTEGER) {
					if (!(Math.floor(field.value) == field.value && $.isNumeric(field.value))) {
						return task.language.invalid_value.replace('%s', '');
					}
				}
				else if (item.f_data_type.value === task.consts.FLOAT || item.f_data_type.value === task.consts.CURRENCY) {
		...
		else if (field.field_name === 'f_calc_lookup_field') {
			if (item.f_calc_item.value && !field.value) {
				return field.field_caption + ' - ' + task.language.value_required;   
			}
		}
		else if (field.field_name === 'f_calc_field') {
			if (item.f_calc_item.value && !field.value) {
				return field.field_caption + ' - ' + task.language.value_required;   
			}
		}
		else if (field.field_name === 'f_calc_op') {
			if (item.f_calc_field.value && !field.value) {
				return field.field_caption + ' - ' + task.language.value_required;   
			}
		}
	}
	...
	function on_before_field_changed(field) {
		let item = field.owner;
		if (field.field_name === 'f_default_lookup_value') {	
			if (new_field(item) && item.f_data_type.value === task.consts.BOOLEAN) {		
			...
			try {
				...
				if (field.field_name === 'f_object') {
					...
					if (item.f_object.value) {
						...
						if (item.f_data_type.value === task.consts.KEYS) {
					...
					update_default_value(item, field);
				}
				else if (field.field_name === 'f_object_field') {
					item.f_object_field1.value = null;
					item.f_object_field2.value = null;
					...
					item.f_default_value.value = field.display_text.toLowerCase();
					if (item.f_lookup_values.value) {
						item.f_default_value.text = field.data;
					}
					else if (!item.f_default_value.value) {
						item.f_default_value.value = null;
					}
				}
				else if (field.field_name === 'f_calc_item') {
					item.f_object.value = null;
					item.f_object_field.value = null;
					item.f_object_field1.value = null;
					item.f_object_field2.value = null;
					...
					item.f_calc_field.value = null;
					item.f_calc_op.value = null;
					item.f_calc_lookup_field.value = null;
					if (field.value) {
						let owner_id = item.owner.id.value;
						if (item.owner.f_copy_of.value) {
							owner_id = item.owner.f_copy_of.value;
						}
						let fields = task.sys_fields.copy({handlers: false});
						fields.set_where({
							owner_rec_id: item.f_calc_item.value,
							f_object: owner_id,
							f_master_field__isnull: true
						...
						if (fields.rec_count === 1) {
							item.f_calc_lookup_field.value = fields.id.value;
							item.f_calc_lookup_field.lookup_value = fields.f_field_name.value;
						}
					}
				}
				else if (field.field_name === 'f_calc_field') {
					item.f_calc_op.value = null;
					if (lookup_item) {
						if (lookup_item.f_data_type.value === task.consts.INTEGER) {
							item.f_data_type.value = task.consts.INTEGER;
						}
						else if (lookup_item.f_data_type.value === task.consts.FLOAT) {
							item.f_data_type.value = task.consts.FLOAT;
						}
						else if (lookup_item.f_data_type.value === task.consts.CURRENCY) {
							item.f_data_type.value = task.consts.CURRENCY;
						}
					}
				}
				else if (field.field_name === 'f_calc_op') {
					if (!item.f_calc_field.value) {
						item.f_calc_op.value = null;
					}
					else if (item.f_calc_op.display_text.toUpperCase() === 'COUNT') {
						item.f_data_type.value = task.consts.INTEGER;
					}
					item.f_alignment.value = task.consts.ALIGN_RIGHT;
				}
	...
	function update_default_value(item, field) {
		if (item.edit_form && item.f_default_value) {
			...
			else if (item.f_data_type.value === task.consts.DATETIME) {
				item.f_default_lookup_value.lookup_values = [[0, ''], [1, 'CURRENT DATETIME']];
			}
			else if (item.f_data_type.value === task.consts.BOOLEAN) {
				item.f_default_lookup_value.lookup_values = [[0, 'FALSE'], [1, 'TRUE']]
			}
			...
			if (field) {
				...
				if (item.f_data_type.value === task.consts.BOOLEAN) {
	...
	function update_read_only(item) {
		if (!item.f_data_type.value) {
			item.f_read_only.value = false;
			item.f_read_only.read_only = true;
			item.f_required.value = false;
			item.f_required.read_only = true;
		}
		else {
			item.f_read_only.read_only = false;		
			item.f_required.read_only = false;
		}
		if (!new_field(item) && !item.owner.f_virtual_table.value && !item.task._manual_update) {
			...
			item.f_enable_typehead.read_only = 
				!(item.f_object.value && !item.f_master_field.value || item.f_lookup_values.value);
			// item.f_calc_item.read_only = !item.f_calc_item.value;
			item.f_calc_field.read_only = !item.f_calc_item.value;
			item.f_calc_op.read_only = !item.f_calc_item.value;
			if (item.task.db_params.name === 'MSSQL') {
				item.f_field_name.read_only = true;
			}
		}
		else {
			item.f_data_type.read_only = item.f_data_type.value && (item.f_object.value || item.f_lookup_values.value);
			if (item.f_data_type.value === task.consts.TEXT && !item.f_object.value) {
				item.f_size.read_only = false;
			}
			else {
				item.f_size.read_only = true;
			}
			item.f_calc_item.read_only = false;
			item.f_calc_field.read_only = false;
			item.f_calc_op.read_only = false;
	...
	function update_lookup_attr(item, is_new) {
		...
		if (item.f_data_type.value === task.consts.KEYS && item.f_object.value) {
	...
	function get_alignment(item) {
		var data_type = item.f_data_type.value,
			result;
		if (data_type === task.consts.INTEGER ||
			data_type === task.consts.FLOAT ||
			data_type === task.consts.CURRENCY) {
			result = task.consts.ALIGN_RIGHT;
		}
		else if (data_type === task.consts.DATE ||
			data_type === task.consts.DATETIME) {
			result = task.consts.ALIGN_CENTER;
		}
		else {
			result = task.consts.ALIGN_LEFT;
		}
	...
	function can_delete_field(item) {
		...
		clone = item.clone()
		clone.each(function(c) {
			if (item.f_object.value && c.f_object.value === item.f_object.value && 
				c.f_master_field.value === item.id.value) {
			...
			if (c.id.value === item.id.value) {		
				if (c.id.value === item.owner.f_primary_key.value) {
		...
		if (!error && !new_field(item)) {
			error = task.sys_fields.server('server_can_delete_field', [item.id.value]);
		}
		return error
	}
	...
	function on_before_post(item) {
		...
		item.task_id.value = task.item_tree.task_id.value;
		if (!item.id.value) {
	...
	this.update_read_only = update_read_only;
